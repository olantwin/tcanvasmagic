...
class CanvasMagics(Magics):
    ...
    @argument(
        '--name', action='store', type=str, default=None,
    ...
    def canvas(self, line, cell=None):
        ...
        constructor = "ROOT.TCanvas('{uid}', '{uid}', {width}, {height})".format(
            uid=uid,
            width=width,
            height=height
        ) if size else "ROOT.TCanvas()"
        canvas_name = args.name if args.name else '__c_{uid}__'.format(uid=uid)
        cell = canvas_name + " = {constructor}\n".format(constructor=constructor) + cell + "\n" + canvas_name + ".Draw()"
        if args.save:
